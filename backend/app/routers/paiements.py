...
def process_payment(payload: PaiementCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    if current_user.role.nom not in ["admin", "caissier"]:
        raise HTTPException(status_code=403, detail="Accès réservé aux caissiers")
    result = PaymentService.process_payment(db, payload.commande_id, payload.montant, payload.mode_paiement)
